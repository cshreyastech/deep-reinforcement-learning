...
if __name__ == '__main__':
  ...
  modle_name_pkl = 'acrobot_expected_sarsa.pkl'
  if load == False:
    Q = {}
    for state in states:
  ...
  for i in range(n_games):
    ...
    while not done:
      ...
      actions_.remove(action_)
      expected_sarsa_ = Q[state_, action_] * 1 - eps + (eps / nA)

      for a_ in actions_:
        expected_sarsa_ += Q[state_, a_] * eps / nA 

      Q[state, action] += alpha * (reward + gamma * expected_sarsa_ - Q[state, action])
