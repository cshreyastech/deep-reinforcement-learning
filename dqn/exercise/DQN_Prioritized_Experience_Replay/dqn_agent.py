...
class Agent():
  ...
  def learn(self, experiences, gamma):
    ...
    """
    states, actions, rewards, next_states, dones, importance = experiences
    ...
    errors = q_expected - q_target
    errors_ = errors.view(errors.shape[0])

    # TBD - to replace with actual importance
    # importance = torch.randn_like(errors_)
    importance = importance**(1 - self.eps)
    # print("importance", importance.shape)

    loss = torch.mean(torch.square(errors_) * importance)
    ...
    # print("errors_", type(errors_.cpu().detach().numpy()))
    self.memory.set_priorities(errors_.cpu().detach().numpy())
