...
from replay_buffer_tbd import ReplayBuffer

from dueling_model_tbd import DuelingQNetwork
  ...
  def learn(self, experiences, gamma):
    ...
    with torch.no_grad():
      ...
      ### Use of Fixed Q-Target
        ...
        # calculate the pair action/reward for each of the next_states
        next_action_rewards = self.qnetwork_target(next_states) #(batch_size x action_size)

        # select the maximum reward for each of the next actions
        target_rewards = next_action_rewards.max(dim=1, keepdim=True)[0] #(batch_size x 1)
