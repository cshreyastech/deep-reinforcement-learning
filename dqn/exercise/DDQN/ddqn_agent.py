...
class Agent():
  ...
  def learn(self, experiences, gamma):
    ...
    ##########
    q_local_next = self.qnetwork_local(next_states) #(BATCH_SIZE x action_size)
    greedy_action_local = q_local_next.max(dim=1, keepdim=True)[1] #(BATCH_SIZE x 1)
    ...
    q_target_next_max = q_target_next.gather(1, greedy_action_local) #(BATCH_SIZE x 1)
